...
public class Boss1 : MonoBehaviour
{
    ...
    private void SetBossAttributes()
    {
    ...
    IEnumerator SummonMonstersRoutine()
    {
        while (true)
        {
            yield return new WaitForSeconds(25f);
            SummonMonsters(12); // เรียก Monster จำนวน 12 ตัว
        }
    }
    ...
    {
        SetBossAttributes(); // เรียกฟังก์ชันแยกเมื่อโหลดฉากเสร็จ
    }
    private void SummonMonsters(int count)
    {
        for (int i = 0; i < count; i++)
        {
            GameObject monster = Instantiate(Resources.Load<GameObject>("NormalMonster"));
            monster.transform.position = transform.position; // ให้เกิดใกล้ ๆ กับ Boss
            Debug.Log("Summoned Monster " + (i + 1));
        }
    }
    ...
    /*private void OnEnable()
    {
        ...
        {
            skillManager.allEnemies.Add(this); // เพิ่มตัวเองในลิสต์เมื่อเปิดใช้งาน
        }
    }
    ...
    private void OnDisable()
    {
        ...
        if (skillManager != null)
        {
            skillManager.allEnemies.Remove(this); // ลบตัวเองออกจากลิสต์เมื่อปิดใช้งาน
        }
    }*/
    // สกิล1
